...
namespace Nez
{
	...
	public class ImageMaskTransition : SceneTransition
	{
		...
		public ImageMaskTransition( Func<Scene> sceneLoadAction, Texture2D maskTexture ) : base( sceneLoadAction, true )
		{
			...
			_maskRenderTarget = new RenderTarget2D( Core.CoreGraphicsDevice, Screen.Width, Screen.Height, false, SurfaceFormat.Color, DepthFormat.None );
		...
		{
			GraphicsDeviceExt.SetRenderTarget(Core.CoreGraphicsDevice, _maskRenderTarget );
		...
		protected override void TransitionComplete()
		{
			...
			Core.GlobalContent.UnloadAsset<Texture2D>( _maskTexture.Name );
