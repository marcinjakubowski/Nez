...
namespace Nez
{
	public class RenderableComponentList
	{
		...
		static RenderableComparer _compareUpdatableOrder = new RenderableComparer();
		...
		public void UpdateLists()
		{
			...
			{
				_components.Sort( _compareUpdatableOrder );
			...
			if( _unsortedRenderLayers.Count > 0 )
			{
				for( int i = 0, count = _unsortedRenderLayers.Count; i < count; i++ )
					_componentsByRenderLayer[_unsortedRenderLayers[i]].Sort( _compareUpdatableOrder );
