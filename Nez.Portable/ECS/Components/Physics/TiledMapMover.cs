	...
	#endif
	{
		...
		public void TestCollisions( ref Vector2 motion, Rectangle boxColliderBounds, CollisionState collisionState )
		{
			...
			// next, check movement in the vertical dir
			{
				...
				if( TestMapCollision( sweptBounds, direction, collisionState, out collisionResponse ) )
				{
					...
					if( collisionState.Below && collisionState.LastGroundTile != null && collisionState.LastGroundTile.IsSlope() )
						collisionState.SlopeAngle = MathHelper.ToDegrees( (float)Math.Atan( collisionState.LastGroundTile.GetSlope() ) );
				}
				else
				{
		...
		bool TestMapCollision( Rectangle collisionRect, Edge direction, CollisionState collisionState, out int collisionResponse )
		{
			...
			for( var i = 0; i < _collidingTiles.Count; i++ )
			{
				...
				if( direction.IsHorizontal() && collisionState.LastGroundTile != null && collisionState.LastGroundTile.IsSlope() && _collidingTiles[i].IsSlope() )
					return false;
				...
				if( collisionState.LastGroundTile != null && direction == Edge.Bottom )
				{
					...
					if( ( collisionState.LastGroundTile.IsSlope() && _collidingTiles[i].IsSlope() ) || ( !collisionState.LastGroundTile.IsSlope() && isHighSlopeNearest ) )
					{
						// store off our last ground tile if we collided below
						collisionState.LastGroundTile = _collidingTiles[i];
						return true;
