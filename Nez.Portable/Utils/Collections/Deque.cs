...
namespace System.Collections.Generic
{
	...
	public class Deque<T> : IList<T>
	{
		...
		int _capacityClosestPowerOfTwoMinusOne;
		...
		public int Capacity
		{
			...
			set
			{
				...
				else if( null != _buffer && value == _buffer.Length )
				{
					return;
				}
				...
				_buffer = newBuffer;
				_startOffset = 0;
				this._capacityClosestPowerOfTwoMinusOne = powOfTwo - 1;
			}
		}
		...
		int ToBufferIndex( int index )
		{
			...
			bufferIndex = ( index + this._startOffset )
			& this._capacityClosestPowerOfTwoMinusOne;
		...
		{
			this._startOffset = ToBufferIndex( value );
		...
		public IEnumerator<T> GetEnumerator()
		{
			...
			if( this._startOffset + this.Count > this.Capacity )
			{
				for( int i = this._startOffset; i < this.Capacity; i++ )
				{
			...
			{
				int endIndex = this._startOffset + this.Count;
				for( int i = this._startOffset; i < endIndex; i++ )
				{
					yield return _buffer[i];
				}
			}
		...
		void ClearBuffer( int logicalIndex, int length )
		{
			...
			if( offset + length > this.Capacity )
			{
				...
				Array.Clear( this._buffer, offset, len );

				len = ToBufferIndex( logicalIndex + length );
				Array.Clear( this._buffer, 0, len );
			}
			else
			{
				Array.Clear( this._buffer, offset, length );
			}
		}
		...
		public void CopyTo( T[] array, int arrayIndex )
		{
			...
			if( 0 != this._startOffset
			    && this._startOffset + this.Count >= this.Capacity )
			{
				int lengthFromStart = this.Capacity - this._startOffset;
				int lengthFromEnd = this.Count - lengthFromStart;

				Array.Copy(
					_buffer, this._startOffset, array, 0, lengthFromStart );
			...
			{
				Array.Copy(
					_buffer, this._startOffset, array, 0, Count );
			}
		}
		...
		{
			EnsureCapacityFor( 1 );
			_buffer[ToBufferIndex( this.Count )] = item;
			IncrementCount( 1 );
		}
		...
		public T RemoveFront()
		{
			...
			T result = _buffer[this._startOffset];
